...
export class MessageBridge {
    ...
    handleWindowMessage(event) {
        ...
        if (action === 'UI_READY') {
            ...
            // Auto-open settings if flagged from floating bubble

            chrome.storage.local.get(['gemini_open_settings'], (res) => {

                if (res.gemini_open_settings) {

                    chrome.storage.local.remove('gemini_open_settings');
        ...
        if (action === 'GET_EXPLAIN_PAGE_CONTEXT') {

            chrome.storage.local.get(['geminiExplainPageContextEnabled'], (res) => {

                const val = res.geminiExplainPageContextEnabled !== false;

                this.frame.postMessage({ action: 'RESTORE_EXPLAIN_PAGE_CONTEXT', payload: val });
        ...
        if (action === 'GET_BUBBLE_ENABLED') {

            chrome.storage.local.get(['gemini_bubble_enabled'], (res) => {

                const val = res.gemini_bubble_enabled !== false;

                this.frame.postMessage({ action: 'RESTORE_BUBBLE_ENABLED', payload: val });
        ...
        if (action === 'SAVE_EXPLAIN_PAGE_CONTEXT') this.state.save('geminiExplainPageContextEnabled', payload);
